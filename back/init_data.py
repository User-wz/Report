...
from app.models.users import Org, User
...
async def init_group():
    ...
    """
    try:
        if await Org.filter(name=BUILT_IN_GROUP["name"]).exists():
            print(f"内置用户组已经存在: {BUILT_IN_GROUP['name']}")
        else:
            group = await Org.create(**BUILT_IN_GROUP)
...
async def init_user():
    ...
    """
    try:
        if await User.filter(username=BUILT_IN_USER["username"]).exists():
            print(f"内置系统管理员已经存在: {BUILT_IN_USER['username']}")
        else:
            BUILT_IN_USER["group"] = await Org.get(name=BUILT_IN_GROUP["name"])
            user = await User.create(**BUILT_IN_USER)
...
async def init_db():
    await Tortoise.init(
        db_url="postgres://postgres:difyai123456@192.168.223.134:5432/report",
        modules={"models": ["app.models"]}  # 替换为你的模型所在路径
    )
    await Tortoise.generate_schemas()  # 可选：自动创建表结构
