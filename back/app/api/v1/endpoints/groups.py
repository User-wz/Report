...
from app.models.users import Org
from app.schemas.users import GroupIn, GroupOut
...
async def example_get_groups():
    # TODO 示例：获取用户组列表
    return [GroupOut.from_orm(group) for group in await Org.all()]
...
async def example_get_group(group_id: int):
    # TODO 示例：获取指定 ID 用户组
    return await Org.filter(id=group_id).first()
...
async def example_update_group(group_id: int, group_in: GroupIn):
    # TODO 示例：更新指定 ID 用户组
    group = await Org.get(id=group_id)
    if not group:
